...
import {
  ...
  CircleDollarSign, Factory, Flame, Gauge, HandCoins, Heart, Lightbulb, LockKeyhole,
  PackageCheck, Pause, RotateCcw, ShieldCheck, Sparkles, Star, Store, TrendingUp, Truck, Zap,
} from 'lucide-react';
...
function ResultPanel({ option, round, onNext }: { option: Option; round: Round; onNext: () => void }) {
  return <div className="result-panel slide-in mt-8 overflow-hidden rounded-3xl border-2"><div className="flex flex-wrap items-center justify-between gap-4 border-b border-white/20 px-5 py-4 sm:px-7"><div className="flex items-center gap-3"><div className="grid h-9 w-9 place-items-center rounded-full bg-[var(--mint)] text-[var(--ink)]"><Check className="h-5 w-5" /></div><div><div className="font-mono text-[9px] uppercase tracking-[.18em] text-[#b5f2e8]">keputusan dikunci</div><div className="font-display font-bold">{option.title}</div></div></div><div className="rounded-full bg-white/15 px-3 py-1 font-mono text-[10px] text-[#e3f7ff]">profit simulasi +{rupiah(option.profit)}</div></div><div className="grid gap-5 px-5 py-5 sm:grid-cols-[1fr_auto] sm:px-7"><div><div className="mb-2 flex items-center gap-2 font-mono text-[10px] uppercase tracking-[.17em] text-[var(--yellow)]"><Lightbulb className="h-3.5 w-3.5" /> kenapa ini penting</div><p className="max-w-2xl text-sm leading-relaxed">{option.rationale}</p></div><button data-testid="button-next-round" onClick={onNext} className="button-yellow group self-end rounded-xl border-2 px-4 py-3 font-display text-sm font-bold transition hover:-translate-y-1 sm:min-w-[155px]">Lanjut ronde {round < 5 ? round + 1 : 'akhir'} <ArrowRight className="ml-1 inline h-4 w-4 transition-transform group-hover:translate-x-1" /></button></div></div>;
}
...
function GameScreen({ name, brand, onFinish, onRestart }: { name: string; brand: string; onFinish: (score: number, profit: number, reputation: number) => void; onRestart: () => void }) {
  ...
  const choose = (option: Option) => {
    if (selected) return;
    setSelected(option.id); setCash((value) => value - option.cost + option.profit); setProfit((value) => value + option.profit);
