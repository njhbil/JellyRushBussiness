...
const optionsByRound: Record<Round, Option[]> = {
  ...
  2: [
    { id: 'shift', title: 'Tambah shift malam', eyebrow: 'Andalkan tim inti', description: 'Bayar lembur dan dorong kapasitas pabrik.', cost: 1850000, profit: 5150000, capacity: 200, inventory: -100, reputation: 4, rationale: 'Shift tambahan memperluas kapasitas tanpa investasi mesin. Biaya tenaga kerja naik, tetapi kamu mempertahankan kontrol kualitas dan reputasi.', icon: BatteryCharging, tone: 'pink' },
  ...
  3: [
    ...
    { id: 'backup', title: 'Aktifkan pemasok cadangan', eyebrow: 'Adaptif', description: 'Bayar premium untuk bahan yang tersedia sekarang.', cost: 900000, profit: 1950000, capacity: 100, inventory: -50, reputation: 1, rationale: 'Pemasok cadangan mengajarkan pentingnya redundansi. Biaya sedikit lebih tinggi, tetapi pabrik tetap bergerak dan pelanggan tidak menunggu.', icon: Truck, tone: 'pink' },
    { id: 'pause', title: 'Jeda produksi', eyebrow: 'Main aman', description: 'Gunakan stok yang ada dan tunggu situasi reda.', cost: 700000, profit: 300000, capacity: -50, inventory: 20, reputation: -3, rationale: 'Mesin berhenti tapi gaji jalan terus: kasmu bocor Rp400 ribu. Jeda menghemat bahan, tetapi pelanggan yang menunggu punya biaya tersembunyi.', icon: Pause, tone: 'sand' },
  ],
  4: [
    ...
    { id: 'ads', title: 'Bakar duit buat iklan', eyebrow: 'Biar viral', description: 'Habiskan budget besar buat endorse dan ads sekaligus.', cost: 3200000, profit: 1400000, capacity: 0, inventory: -20, reputation: -1, rationale: 'Iklan tanpa kapasitas siap cuma bikin antrean kecewa: kas jebol Rp1,8 juta dan ada pembeli yang kapok. Momentum harus ditangkap pabrik dulu, baru kamera.', icon: Flame, tone: 'sand' },
  ],
  5: [
    ...
    { id: 'loan', title: 'Utang buat ekspansi', eyebrow: 'Nekat', description: 'Pinjam dana besar demi tampil maksimal di depan distributor.', cost: 4500000, profit: 2600000, capacity: 120, inventory: -60, reputation: -2, rationale: 'Gengsi dibayar pakai bunga: kas jebol Rp1,9 juta dan distributor justru ragu sama pabrik yang gali lubang. Ekspansi sehat itu dari kas, bukan dari utang.', icon: HandCoins, tone: 'sand' },
  ],
};
...
const bestChoice: Record<Round, { id: string; why: string; trapId?: string; trapLine?: string }> = {
  ...
  2: { id: 'shift', why: 'Shift malam nambah kapasitas paling besar tanpa ngunci modal di barang.', trapId: 'reject', trapLine: 'Untungnya kamu nggak pilih Tolak pesanan — keluar 600 ribu, reputasi anjlok 8.' },
  3: { id: 'backup', why: 'Satu-satunya cara mesin tetap jalan hari ini juga. Redundansi ngalahin gengsi.', trapId: 'pause', trapLine: 'Untungnya kamu nggak pilih Jeda produksi — mesin tidur, kas bocor 400 ribu.' },
  4: { id: 'scale', why: 'Demand +40% cuma bisa dimakan sama kapasitas siap. Ini momennya, bukan iklannya.', trapId: 'ads', trapLine: 'Untungnya kamu nggak kejebak iklan — bakar 3,2 juta, balik 1,4 juta.' },
  5: { id: 'grand', why: 'Kas dan reputasimu dibangun 4 level buat momen ini. All in yang diperhitungkan.', trapId: 'loan', trapLine: 'Untungnya kamu nggak ngutang — bunga makan kas 1,9 juta.' },
};
